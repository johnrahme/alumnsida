@if(Auth::check())
                                <li  id = "start2" class> {{link_to('/','Start')}}</li>
                                <li  id = "events2"> {{link_to_route('events','Event')}}</li>
                                <li  id = "admin2"> {{link_to_route('admin','Styrelsemedlemmar')}}</li>
                                <li  id = "menu2"> {{link_to_route('menu.index','Menyer')}}</li>
                                    		...
                                    		@if(count($subMenusView)!= 0)
                                             ...
                                             <ul class="dropdown-menu">
     												@foreach($subMenusView as $subMenu)
                                                       <li id = "{{$subMenu->id}}2">{{link_to_route('menu.dyn',$subMenu->name, $subMenu->url)}}
                                                       </li>
                                                     @endforeach
                                             </ul>
                                             @else
