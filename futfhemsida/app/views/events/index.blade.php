        ...
        <div class="col-sm-4">
            <div class="panel panel-default">
                <div class="panel-body" style="padding-top: 0">
                    ...
                    <button class="btn btn-sm btn-default changeable" id="show" name="show" value="hidden">Visa tidigare
                        evenemang
                    </button>
                    @if(Auth::check())
            ...
            <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="false">
                ...
                @foreach($events as $key => $currEvent)
                    <div class="panel panel-default @if(time()>strtotime($currEvent->dateTimeTo)) hideable @endif">
                                ...
                                <div class="col-xs-8 fixed">
                                    <h4 class="panel-title">
                                        <a data-toggle="collapse" data-parent="#accordion"
                                           href="#collapse{{$currEvent->id}}" aria-expanded="true"
                                           aria-controls="collapse{{$currEvent->id}}">
                                            {{$currEvent->name}}
                                        </a>
                                        <small>{{date('Y-m-d', strtotime($currEvent->dateTimeFrom))}}
                                            Kl. {{date('H:i', strtotime($currEvent->dateTimeFrom))}}</small>
                                    </h4>
                                </div>
                        ...
                        <div id="collapse{{$currEvent->id}}" class="panel-collapse @if($key == 0) in @endif collapse"
                             role="tabpanel" aria-labelledby="heading{{$currEvent->id}}">
