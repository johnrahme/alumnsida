    ...
    <div class="panel panel-default @if(time()>strtotime($currEvent->dateTimeTo)) hideable @endif">
        <div class="panel-heading" role="tab" id="heading{{$currEvent->id}}">
            <h4 class="panel-title">
                <a data-toggle="collapse" data-parent="#accordion" href="#collapse{{$currEvent->id}}"
                   aria-expanded="true" aria-controls="collapse{{$currEvent->id}}">
                    {{$currEvent->name}}
                </a>
        ...
        <div id="collapse{{$currEvent->id}}" class="panel-collapse @if($key == 0) in @endif collapse" role="tabpanel"
             aria-labelledby="heading{{$currEvent->id}}">
