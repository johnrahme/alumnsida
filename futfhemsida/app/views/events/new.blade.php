...
@section('panelThree')
    ...
    @if(Input::old('reg'))
        <div id="registrering">
                    ...
                    <div id="wrapper">
                        ...
                        @if(Input::old('extras'))
                            @foreach(Input::old('extras') as $key => $text)
                                @if($text != "")
                                    <div><input type="text" value={{$text}} name="extras[]" id="extras[]"/><a href="#"
            ...
            @section('after')
                <p>
                    <button id="save" class="btn btn-primary" onclick="save()" type="button">Add event
                    </button> {{ link_to_route('events', 'Cancel', array(), array('class'=>'btn btn-default')) }}</p>
                {{Form::close()}}
            @stop
            ...
            @section('scripts')
                ...
                {{HTML::script('plugins/summernote/custom/onImageUpload.js')}}
                 ...
                 onImageUpload("owncloud/styrelsen/files/images/events","{{url('events/imgstore')}}");
                </script>
